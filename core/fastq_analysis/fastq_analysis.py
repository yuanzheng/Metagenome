...
from core.io_process.csv_handler import CSVHandler
from utils.thread_utilies.fastq_analysis_thread import AnalysisTask
from utils.thread_utilies.signals import TaskType, signals
...
class FastQAnalysisProcessor:
    ...
    def __init__(self, tab_widget):
        ...
        self._table_header = []
        self.listWidget_fastq_analysis = tab_widget.findChild(
            QListWidget, "listWidget_fastq_analysis"
        )
        self.listWidget_fastq_analysis.itemChanged.connect(
            self.on_item_changed
        )  # 连接状态变化事件
        self.button_analysis_start = tab_widget.findChild(
            QPushButton, "button_analysis_start"
        )
        self.button_analysis_start.clicked.connect(self.start_analysis)
        self.checkBox_total_bases = tab_widget.findChild(
            QCheckBox, "checkBox_total_bases"
        )
        self.checkBox_total_reads = tab_widget.findChild(
            QCheckBox, "checkBox_total_reads"
        )
        ...
        self.tableWidget_fastq_analysis = tab_widget.findChild(
            QTableWidget, "tableWidget_fastq_analysis"
        )
        self.progressBar_fastq_analysis = tab_widget.findChild(
            QProgressBar, "progressBar_fastq_analysis"
        )
        ...
        self._thread_progress = 0  # 每个thread task 在progress bar上%比
    ...
    def start_analysis(self):
        ...
        self._table_header = self._init_table_header()

        csv_dir = (
            Path(config.FASTQ_DATA_DIRECTORY) / config.FASTQ_ANALYSIS_REPORT_DIRECTORY
        )
        self._csv_handler = CSVHandler(csv_dir, "stats.csv")
        ...
        for file_id in selected_items:
            ...
            file_path = self._raw_fastq_filenames[file_id]
            thread = AnalysisTask(
                file_id,
                file_path,
            ...
            self._thread_pool.submit_task(task_id=file_id, task=thread)
            thread_counter += 1
            self._update_progress(1)

        self.logger.debug("Total %s threads", thread_counter)
        self._thread_progress = (
            (100 - thread_counter) / thread_counter if thread_counter != 0 else 0
        )
        self.logger.debug(
            "To complete a task, progress should increase: %s", self._thread_progress
        )
    ...
    def on_task_error(self, file_id, task_type, error_msg):
        ...
        for row in range(self.tableWidget_fastq_analysis.rowCount()):
            if self.tableWidget_fastq_analysis.item(row, 0).text() == file_id:
                for index in range(1, len(self._table_header)):
                    self.logger.error(
                        "Error, column for %s: %s", self._table_header[index], error_msg
                    )
        ...
        if self._thread_pool.alive_thread_counter() == 0:
    ...
    def on_result_ready(self, file_id, task_type, result):
        ...
        if not result:
            self.logger.error(
                "Failed to get FASTQ analysis result for file: %s", file_id
            )
            return
        ...
        if self._csv_handler is not None:
            self._csv_handler.append_result(file_id, self._table_header, result)

        """Upgrade the progress bar"""
        self._update_progress(self._thread_progress)
    ...
    def list_fastq_files(self, fastq_file_directory):
        ...
        try:
            ...
            self.logger.debug(
                "Number of fastq files: %s", self.listWidget_fastq_analysis.count()
            )
            return True
    ...
    def _get_selected_items(self):
        ...
        for index in range(1, num_items):
            ...
            if state == Qt.Checked:
                self.logger.debug(
                    "--- Selected items %s",
                    self.listWidget_fastq_analysis.item(index).text(),
                )
    ...
    def _init_table_header(self):
        ...
        self.tableWidget_fastq_analysis.horizontalHeader().setSectionResizeMode(
            QHeaderView.Stretch
        )
    ...
    def _init_table_row(self, selected_items):
        ...
        for row, item in enumerate(selected_items):
            self.logger.debug(
                "Initialize table for the selected items: %s and %s", row, item
            )
            for col in range(len(self._table_header)):
                if col == 0:
                    # 样本ID
                    self.tableWidget_fastq_analysis.setItem(
                        row, col, QTableWidgetItem(item)
                    )
                else:
                    # 初始状态
    ...
    def _task_skipped(self, existing):
        ...
        for row in range(self.tableWidget_fastq_analysis.rowCount()):
            ...
            self.logger.debug(
                "Table widget in row %s for file: %s",
                row,
                self.tableWidget_fastq_analysis.item(row, 0).text(),
            )
            if (
                existing[self._table_header[0]]
                == self.tableWidget_fastq_analysis.item(row, 0).text()
            ):
                for index in range(1, len(self._table_header)):
    ...
    def _statistics_exist(self, file_id):
        if self._csv_handler.has_file_id(file_id):
            existing_one = self._csv_handler.get_existing_data(file_id)
    ...
    def _update_progress(self, increase_value):
        ...
        self.progressBar_fastq_analysis.setValue(self._progress_counter)
