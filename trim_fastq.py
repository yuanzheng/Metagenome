...
def select_adapters():
    ...
    adapters = [f for f in os.listdir(adapter_dir) 
...
def build_custom_params():
    ...
    while True:
        ...
        if choice == '5':
            while True:
                try:
                    values = input("输入SLIDINGWINDOW, 窗口大小和平均质量（用空格分隔）: ").split()
                    if len(values) != 2:
                        raise ValueError
...
def main():
    ...
    # 构建命令
    cmd = [
        'java', '-jar', os.path.join('Trimmomatic-0.36', 'trimmomatic-0.36.jar'),
        read_type,
        '-threads', str(threads),
        '-' + phred
    ]
    ...
    if read_type == 'PE':
        ...
        cmd += [
            se,
            os.path.join(out_dir, f"{base}_trimmed.fq")
        ]
